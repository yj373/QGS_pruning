...
parser.add_argument('--initial_model', default='', type=str, help='path to the pretrained model')
...
def main():
    ...
    if os.path.isfile(args.initial_model):
        print('load the initial model from {}'.format(args.initial_model))
        model_dict = torch.load(args.initial_model)
    ...
    S = torch.ones(1, dtype=torch.float32, device='cuda') * 0.01
    ...
    T_list = [args.ce_target] # targets for L(W) and distance
    print('Targets >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    print('CE loss: {}'.format(T_list[0]))
    print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    ...
    for epoch in range(args.prune_pretrain):
        ...
        obj_meters, _, train_accuracy = train(epoch, model, train_loader, optimizer, criterions[0], loss_name_list[0], T_list, num_classes, 
            S=S, print_frep=100, prune_alpha=args.prune_alpha, structured=args.structured, order=order, QGS_lr_constant=args.QGS_lr_constant, 
        ...
        if args.soft_prune_cycle < args.prune_pretrain and epoch % args.soft_prune_cycle == 0:
            # Soft pruning
            print('model sum: ', check_sum(model))
            soft_pruning(model, args.prune_alpha, train_loader, test_loader, criterions, loss_name_list, num_classes, epoch, 
                savedir=args.savedir, structured=args.structured, path='results/temp.pth', recover=False, order=order, finetune_best=args.finetune_best)
            print('model sum: ', check_sum(model))
    ...
    print('Hard pruned model test accuracy before finetuning: {:.3f}, test loss: {:.4f}, train accuracy: {:.3f}, train loss: {:.4f}'.format(
...
def train(epoch, model, train_loader, optimizer, criterion, loss_name, T_list, num_classes, 
    S=None, print_frep=100, prune_alpha=0.5, structured=False, order=1, k=1e-3, QGS_lr_constant=0.05, QGS_lr_max=0.1):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        # if not structured:
        #     prune_method = prune.L1Unstructured(amount=prune_alpha)
        # else:
        #     prune_method = prune.LnStructured(amount=prune_alpha, n=order, dim=1)

        # distance = compute_pruned_distance(model, prune_method, structured, order=order, k=k)
        obj_losses = [original_task_loss]
        ...
        optimizer.step()
        if i % print_frep == 0:
            log = 'step: {}, loss(original): {:.4f} ({:.4f}), train loss: {:.4f} ({:.4f}), train accuracy(original): {:.3f}({:.3f}), lr: {:.5f}'.format(
                i, original_loss_meter.val, original_loss_meter.avg, train_loss_meter.val, train_loss_meter.avg, accuracy_meter.val, accuracy_meter.avg,
                optimizer.param_groups[0]['lr']
            )
            print(log)

    # Update the learning rate based on QGS theory
    divider = 2 * (original_loss_meter.avg) - sum(T_list) + torch.abs(S[0]).data
    ...
    log += '{} loss (original): {:.4f}, '.format(loss_name, original_loss_meter.avg)
    ...
    return [original_loss_meter], train_loss_meter.avg, accuracy_meter.avg

def soft_pruning(model, prune_alpha, train_loader, test_loader, criterions, loss_name_list, num_classes, epoch, structured=False, path='results/temp.pt',
    recover=False, order=1, finetune_best=False, savedir='results'):
    ...
    if finetune_best:
        ...
        if is_best:
            ...
            shutil.copy(path, os.path.join(savedir, 'best_sf_pretrain.pt'))
