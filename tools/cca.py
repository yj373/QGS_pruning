...
from cca_utils import fftfreq, _irfft, _rfft
...
def pwcca_distance(x: Tensor,
    ...
    """ Projection Weighted CCA proposed in Morcos et al. 2018.
    Args:
        x: input tensor of Shape DxH, where D>H
...
class SimilarityHook(object):
    ...
    def _register_hook(self
        ...
        self.hook_handles.append(self.module.register_forward_hook(hook))
    ...
    def create_hooks(model: nn.Module,
        ...
        hooks1 = SimilarityHook(model1, ['name1', ...])
        hooks2 = SimilarityHook(model2, ['name1', ...])
        with torch.no_grad():
            model1(input)
            model2(input)
