...
class WideBasicCurve(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, stride=1, fix_points=None):
...
class WideResNet(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=10, dropout_rate=0., fix_points=None):
    ...
    def _wide_layer(self, planes, num_blocks, dropout_rate, stride, fix_points=None):
        strides = [stride] + [1] * int(num_blocks - 1)
        layers = []
        for stride in strides:
            layers.append(
                WideBasicCurve(self.in_planes, planes, dropout_rate, stride=stride, fix_points=fix_points)
            )
...
def wrn_2810(num_classes=10, fix_points=None):
    return WideResNet(
        num_classes, depth=28, widen_factor=10, dropout_rate=0., fix_points=fix_points
    )
