import numpy as np
import math
import torch
import torch.nn.functional as F
from torch.nn import Module, Parameter, init
from torch.nn.modules.utils import _pair
from scipy.special import binom
from collections import OrderedDict
from copy import deepcopy

class Bezier(Module):
    def __init__(self, num_bends):
        super(Bezier, self).__init__()
        self.register_buffer(
            'binom',
            torch.Tensor(binom(num_bends - 1, np.arange(num_bends), dtype=np.float32))
        )
        self.register_buffer('range', torch.arange(0, float(num_bends)))
        self.register_buffer('rev_range', torch.arange(float(num_bends - 1), -1, -1))

    def forward(self, t):
        return self.binom * \
               torch.pow(t, self.range) * \
               torch.pow((1.0 - t), self.rev_range)


class PolyChain(Module):
    def __init__(self, num_bends):
        super(PolyChain, self).__init__()
        self.num_bends = num_bends
        self.register_buffer('range', torch.arange(0, float(num_bends)))

    def forward(self, t):
        t_n = t * (self.num_bends - 1)
        return torch.max(self.range.new([0.0]), 1.0 - torch.abs(t_n - self.range))


class WarmStart(Module):
    def __init__(self, num_bends):
        super(WarmStart, self).__init__()
        self.num_bends = num_bends
        # self.register_buffer( 'range', torch.cat([torch.tensor([1.0], dtype=torch.float),torch.zeros(num_bends, dtype=torch.float)]) )

    def forward(self, t, choice=None, include_init=True, return_choice=False):
        if choice is None:
            choice = np.random.choice(self.num_bends-1)+1
        # assert choice<self.num_bends
        coeffs_t = torch.zeros(self.num_bends, dtype=torch.float, requires_grad=False)
        if include_init:
            coeffs_t[0].add_(1.0)
        if choice>0: # if choice<=0, then output intiial solution
            coeffs_t[choice].add_(t)

        if return_choice:
            return coeffs_t, choice
        return coeffs_t, None


class CurveModule(Module):

    def __init__(self, fix_points, parameter_names=()):
        super(CurveModule, self).__init__()
        self.fix_points = fix_points
        self.num_bends = len(self.fix_points)
        self.parameter_names = parameter_names
        self.l2 = 0.0

    def compute_weights_t(self, coeffs_t):
        # print('>>>', self)
        w_t = [None] * len(self.parameter_names)
        self.l2 = 0.0
        for i, parameter_name in enumerate(self.parameter_names):
            for j, coeff in enumerate(coeffs_t):
                if coeff==0:
                    continue
                parameter = getattr(self, '%s_%d' % (parameter_name, j))
                if parameter is not None:
                    # print(parameter_name, j, parameter.shape, parameter.requires_grad)
                    if w_t[i] is None:
                        w_t[i] = parameter*coeff.item()
                    else:
                        w_t[i] += parameter*coeff.item()
                    # print(w_t[i].requires_grad)
            # if w_t[i] is not None:
            #     self.l2 += torch.sum(w_t[i] ** 2)
        return w_t


class Linear(CurveModule):

    def __init__(self, in_features, out_features, fix_points, bias=True):
        super(Linear, self).__init__(fix_points, ('weight', 'bias'))
        self.in_features = in_features
        self.out_features = out_features

        self.l2 = 0.0
        for i, fixed in enumerate(self.fix_points):
            self.register_parameter(
                'weight_%d' % i,
                Parameter(torch.Tensor(out_features, in_features), requires_grad=not fixed)
            )
        # for i, fixed in enumerate(self.fix_points):
            if bias:
                self.register_parameter(
                    'bias_%d' % i,
                    Parameter(torch.Tensor(out_features), requires_grad=not fixed)
                )
            else:
                self.register_parameter('bias_%d' % i, None)
        self.reset_parameters()

    def reset_parameters(self):
        stdv = 1. / math.sqrt(self.in_features)
        for i in range(self.num_bends):
            getattr(self, 'weight_%d' % i).data.uniform_(-stdv, stdv)
            # weight = getattr(self, 'weight_%d' % i)
            # init.kaiming_uniform_(weight, a=math.sqrt(5))
            
            bias = getattr(self, 'bias_%d' % i)
            if bias is not None:
                bias.data.uniform_(-stdv, stdv)
                # fan_in, _ = init._calculate_fan_in_and_fan_out(weight)
                # bound = 1 / math.sqrt(fan_in)
                # init.uniform_(bias, -bound, bound)

    def forward(self, input, coeffs_t):
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.linear(input, weight_t, bias_t)


class Conv2d(CurveModule):

    def __init__(self, in_channels, out_channels, kernel_size, fix_points, stride=1,
                 padding=0, dilation=1, groups=1, bias=True):
        super(Conv2d, self).__init__(fix_points, ('weight', 'bias'))
        if in_channels % groups != 0:
            raise ValueError('in_channels must be divisible by groups')
        if out_channels % groups != 0:
            raise ValueError('out_channels must be divisible by groups')
        kernel_size = _pair(kernel_size)
        stride = _pair(stride)
        padding = _pair(padding)
        dilation = _pair(dilation)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
        self.dilation = dilation
        self.groups = groups

        for i, fixed in enumerate(self.fix_points):
            self.register_parameter(
                'weight_%d' % i,
                Parameter(
                    torch.Tensor(out_channels, in_channels // groups, *kernel_size),
                    requires_grad=not fixed
                )
            )
        # for i, fixed in enumerate(self.fix_points):
            if bias:
                self.register_parameter(
                    'bias_%d' % i,
                    Parameter(torch.Tensor(out_channels), requires_grad=not fixed)
                )
            else:
                self.register_parameter('bias_%d' % i, None)
        self.reset_parameters()

    def reset_parameters(self):
        for i in range(self.num_bends):
            weight = getattr(self, 'weight_%d' % i)
            init.kaiming_uniform_(weight, a=math.sqrt(5))
            bias = getattr(self, 'bias_%d' % i)
            if bias is not None:
                fan_in, _ = init._calculate_fan_in_and_fan_out(weight)
                bound = 1 / math.sqrt(fan_in)
                init.uniform_(bias, -bound, bound)

    def forward(self, input, coeffs_t):
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.conv2d(input, weight_t, bias_t, self.stride,
                        self.padding, self.dilation, self.groups)


class _BatchNorm(CurveModule):
    _version = 2

    def __init__(self, num_features, fix_points, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        super(_BatchNorm, self).__init__(fix_points, ('weight', 'bias'))
        self.num_features = num_features
        self.eps = eps
        self.momentum = momentum
        self.affine = affine
        self.track_running_stats = track_running_stats

        self.l2 = 0.0
        for i, fixed in enumerate(self.fix_points):
            if self.affine:
                self.register_parameter(
                    'weight_%d' % i,
                    Parameter(torch.Tensor(num_features), requires_grad=not fixed)
                )
            else:
                self.register_parameter('weight_%d' % i, None)
        for i, fixed in enumerate(self.fix_points):
            if self.affine:
                self.register_parameter(
                    'bias_%d' % i,
                    Parameter(torch.Tensor(num_features), requires_grad=not fixed)
                )
            else:
                self.register_parameter('bias_%d' % i, None)

            if self.track_running_stats:
                self.register_buffer('running_mean_%d'%i, torch.zeros(num_features))
                self.register_buffer('running_var_%d'%i, torch.ones(num_features))
                self.register_buffer('num_batches_tracked_%d'%i, torch.tensor(0, dtype=torch.long))
            else:
                self.register_parameter('running_mean_%d'%i, None)
                self.register_parameter('running_var_%d'%i, None)
                self.register_parameter('num_batches_tracked_%d'%i, None)
        self.reset_parameters()

    def reset_running_stats(self):
        if self.track_running_stats:
            for i, fixed in enumerate(self.fix_points):
                getattr(self, 'running_mean_%d'%i).zero_()
                getattr(self, 'running_var_%d'%i).fill_(1)
                getattr(self, 'num_batches_tracked_%d'%i).zero_()

    def reset_parameters(self):
        self.reset_running_stats()
        if self.affine:
            for i in range(self.num_bends):
                getattr(self, 'weight_%d' % i).data.uniform_()
                getattr(self, 'bias_%d' % i).data.zero_()
                # init.ones_(getattr(self, 'weight_%d' % i)) # changed in new pytorch version!!
                # init.zeros_(getattr(self, 'bias_%d' % i))

    def _check_input_dim(self, input):
        raise NotImplementedError

    def forward(self, input, coeffs_t):
        self._check_input_dim(input)
        _active = torch.nonzero(coeffs_t)
        choice = _active[len(_active)-1].item()
        temp_rm = getattr(self, 'running_mean_%d'%choice)
        temp_rv = getattr(self, 'running_var_%d'%choice)
        temp_nb = getattr(self, 'num_batches_tracked_%d'%choice)

        exponential_average_factor = 0.0

        if self.training and self.track_running_stats:
            temp_nb += 1
            if self.momentum is None:  # use cumulative moving average
                exponential_average_factor = 1.0 / temp_nb.item()
            else:  # use exponential moving average
                exponential_average_factor = self.momentum
        weight_t, bias_t = self.compute_weights_t(coeffs_t)

        return F.batch_norm(
            input, temp_rm, temp_rv, weight_t, bias_t,
            self.training or not self.track_running_stats,
            exponential_average_factor, self.eps)

    def extra_repr(self):
        return '{num_features}, eps={eps}, momentum={momentum}, affine={affine}, ' \
               'track_running_stats={track_running_stats}'.format(**self.__dict__)

    def _load_from_state_dict(self, state_dict, prefix, metadata, strict,
                              missing_keys, unexpected_keys, error_msgs):
        version = metadata.get('version', None)

        if (version is None or version < 2) and self.track_running_stats:
            # at version 2: added num_batches_tracked buffer
            #               this should have a default value of 0
            for i in range(len(self.fix_points)):
                # getattr(self, 'running_mean_%d'%i).zero_()
                # getattr(self, 'running_var_%d'%i).fill_(1)
                # getattr(self, 'num_batches_tracked_%d'%i).zero_()
                num_batches_tracked_key = prefix + 'num_batches_tracked_%d'%i
                if num_batches_tracked_key not in state_dict:
                    state_dict[num_batches_tracked_key] = torch.tensor(0, dtype=torch.long)

        super(_BatchNorm, self)._load_from_state_dict(
            state_dict, prefix, metadata, strict,
            missing_keys, unexpected_keys, error_msgs)


class BatchNorm2d(_BatchNorm):
    # def __init__(self, num_features, fix_points, eps=1e-5, momentum=0.1, affine=True,
    #              track_running_stats=True):
    #     super(BatchNorm2d, self).__init__(num_features, fix_points, eps=eps, momentum=momentum, affine=affine,
    #              track_running_stats=track_running_stats)
    def _check_input_dim(self, input):
        if input.dim() != 4:
            raise ValueError('expected 4D input (got {}D input)'
                             .format(input.dim()))

# class ReLU(CurveModule):
#     def __init__(self, inplace=True):
#         super(ReLU, self).__init__(inplace=inplace)
#     def forward(self, input, coeffs_t):
#         return F.relu(input), coeffs_t




class CurveNet(Module):
    def __init__(self, curve, architecture, num_bends, fix_start=True, fix_end=True,
                 arch_args=(), arch_kwargs={}):
        super(CurveNet, self).__init__()
        # self.num_classes = num_classes
        self.num_bends = num_bends
        self.fix_points = [fix_start] + [False] * (self.num_bends - 2) + [fix_end]
        
        self.curve = curve
        self.architecture = architecture

        self.l2 = 0.0
        self.coeff_layer = self.curve(self.num_bends)
        self.net = self.architecture(*arch_args, fix_points=self.fix_points, **arch_kwargs)
        self.curve_modules = []
        for module in self.net.modules():
            if issubclass(module.__class__, CurveModule):
                self.curve_modules.append(module)

    def import_base_parameters(self, base_model, index):
        parameters = list(self.net.parameters())[index::self.num_bends]
        base_parameters = base_model.parameters()
        for parameter, base_parameter in zip(parameters, base_parameters):
            parameter.data.copy_(base_parameter.data)
    
    def import_base_state_dict(self, base_state_dict, index=0):
        # new_state_dict = OrderedDict()
        new_state_dict = deepcopy(self.net.state_dict())
        idx_len = len(str(index))+1 # length of suffix, eg. '_10'
        if index==0:
            for name, param in new_state_dict.items():
                if name[-2:]=='_{}'.format(index):
                    # if name[:-2] not in base_state_dict:
                    #     print(name)
                    #     print(base_state_dict.keys())
                    param.data.copy_(base_state_dict[name[:-2]].data)
        else:
            for name, param in new_state_dict.items():
                # if 'weight_%d'%index in name or 'bias_%d'%index in name:
                if name[-4-idx_len:] in ['ight_%d'%index, 'bias_%d'%index]:
                    param.data.copy_(base_state_dict[name[:-idx_len]].data - new_state_dict[name[:-idx_len]+'_0'].data)
                # elif 'mean_%d'%index in name or 'var_%d'%index in name or 'batches_%d'%index in name:
                elif name[-4-idx_len:] in ['mean_%d'%index,'_var_%d'%index,'cked_%d'%index]:
                    param.data.copy_(base_state_dict[name[:-idx_len]].data)
        self.net.load_state_dict(new_state_dict)

    def import_base_buffers(self, base_model):
        for buffer, base_buffer in zip(self.net._all_buffers(), base_model._all_buffers()):
            buffer.data.copy_(base_buffer.data)

    def export_base_parameters(self, base_model, index, tt=False):
        if tt: # WarmStart Module is selected
            coeffs_t = self.coeff_layer(1, choice=index)
            for module, base_module in zip(self.net.modules(), base_model.modules()):
                if issubclass(module.__class__, CurveModule):
                    weight_t, bias_t = module.compute_weights_t(coeffs_t)
                    base_module.weight.data.copy_(weight_t.data)
                    if bias_t is not None:
                        base_module.bias.data.copy_(bias_t.data)
        else:
            parameters = list(self.net.parameters())[index::self.num_bends]
            base_parameters = base_model.parameters()
            for parameter, base_parameter in zip(parameters, base_parameters):
                base_parameter.data.copy_(parameter.data)

    def export_base_state_dict(self, index=0, t=1.0, add_t0=True):
        state_dict = OrderedDict()
        temp_state_dict = self.net.state_dict()
        idx_len = len(str(index))+1 # length of suffix, eg. '_10'
        if add_t0 and index>0:
            for name, param in temp_state_dict.items():
                # if 'weight_0' in name or 'bias_0' in name:
                if name[-4-idx_len:] in ['ight_%d'%index, 'bias_%d'%index]:
                    # state_dict[name[:-idx_len]] = deepcopy(param.data).add_(temp_state_dict[name[:-idx_len]+'_0'].data)
                    state_dict[name[:-idx_len]] = (t*param.data).add_(temp_state_dict[name[:-idx_len]+'_0'].data)
                elif name[-4-idx_len:] in ['mean_%d'%index,'_var_%d'%index,'cked_%d'%index]:
                    # state_dict[name[:-idx_len]] = deepcopy(param.data)
                    state_dict[name[:-idx_len]] = param.data
        else:
            for name, param in temp_state_dict.items():
                if name[-idx_len:]=='_{}'.format(index):
                    state_dict[name[:-idx_len]] = param.data

        return state_dict

    def init_linear(self):
        parameters = list(self.net.parameters())
        for i in range(0, len(parameters), self.num_bends):
            weights = parameters[i:i+self.num_bends]
            for j in range(1, self.num_bends - 1):
                alpha = j * 1.0 / (self.num_bends - 1)
                weights[j].data.copy_(alpha * weights[-1].data + (1.0 - alpha) * weights[0].data)

    def weights(self, t):
        coeffs_t = self.coeff_layer(t)
        weights = []
        for module in self.curve_modules:
            weights.extend([w for w in module.compute_weights_t(coeffs_t) if w is not None])
        return np.concatenate([w.detach().cpu().numpy().ravel() for w in weights])

    def _compute_l2(self):
        self.l2 = sum(module.l2 for module in self.curve_modules)

    def forward(self, input, t=None, choice=None, return_choice=False):
        if t is None:
            # t = input.data.new(1).uniform_()
            t = np.random.uniform()
        # if return_choice:
        coeffs_t, choice = self.coeff_layer(t, choice=choice, return_choice=return_choice) # WarmStart module
        # else:
        #     coeffs_t = self.coeff_layer(t, choice=None)
        output = self.net(input, coeffs_t)
        # self._compute_l2()
        if return_choice:
            return output, choice
        return output


def l2_regularizer(weight_decay):
    return lambda model: 0.5 * weight_decay * model.l2
